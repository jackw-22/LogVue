...
describe('archive watcher path filters', () => {
  it('ignores sqlite index files and transient editor files', () => {
    ...
    expect(shouldIgnoreArchivePath('/archive/.logvue')).toBe(true)
    expect(shouldIgnoreArchivePath('/archive/.logvue/index.sqlite')).toBe(true)
    expect(shouldIgnoreArchivePath('C:\\archive\\.logvue\\index.sqlite-wal')).toBe(true)
